  ...
  const Economy = (() => {
    ...
    const emptyWallet = () => ({ souls: 0, owned: [], daily: "", receipts: 0, updatedAt: 0 });
    const cleanWallet = w => {
    ...
    function claimDaily(wallet, now) {
      ...
      return { ok: true, wallet: { ...w, souls: Math.min(MAX_SOULS, w.souls + DAILY), daily: day }, granted: DAILY };
    }
    function credit(wallet, product) {
      const w = cleanWallet(wallet), n = PACKS[product];
      if (!n) return { ok: false, why: "no-such-product" };
      return { ok: true, wallet: { ...w, souls: Math.min(MAX_SOULS, w.souls + n), receipts: w.receipts + 1 }, granted: n };
    }
    return { ITEMS, PACKS, DAILY, MAX_SOULS, emptyWallet, cleanWallet, dayOf, buy, claimDaily, credit };
  })();
