  ...
  function frame(ts) {
    ...
    let dt = real;
    if (!manual) { if (game.slowmo > 0 && !paused) { game.slowmo -= dt; dt *= 0.3; } update(dt); }
  ...
  // ───────────────────────── test hooks (used by TEST_SPEC.js) ─────────────────────────
  window.SkullToss = {
    debug: {
      ...
      step(sec) { const h = 1 / 120; let t = 0; while (t < sec - 1e-9) { const d = Math.min(h, sec - t); update(d); t += d; } draw(); },
      ...
      state() {
        return { state: game.state, score: game.score, hits: game.hits, stage: game.stage, stageHits: game.stageHits, phase: game.phase, lives: game.lives, slots: game.slots, streak: game.streak,
      ...
      visuals, visualShapes: () => visualShapes(), sling: () => ({ ...sling }), VisualSystem,
      ...
      setSetting(k, v) { settings[k] = v; persist(); Sound.apply(); if (sheet === "settings") renderSettings(); },
