  ...
  Object.assign(window.SkullToss.debug, {
    ...
    targetsFull: () => targets.map(T => ({ ...T })), codex: () => ({ count: codexCount(), total: codexTotal(), seen: realProfile().seen.slice(), archive: ARCHIVE.filter(A => A.open()).map(A => A.id) }), shots(on = true) { if (sandbox) sandbox.shotsOn = on; }, lastShots: () => (skull.shots || []).slice(), shotList: () => SHOTS.map(S => ({ id: S.id, rare: S.rare, cam: S.cam })),
