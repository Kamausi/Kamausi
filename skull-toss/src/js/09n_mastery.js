  ...
  const MASTERY = {
    shot: { tiers: [1, 10, 25, 60], pay: [100, 300, 750, 1500], ids: () => SHOT_IDS.slice(), have: id => profile.shots[id] || 0, name: id => t(`shot.${id}.name`), sub: id => t(`shot.${id}.desc`) },
    map:  { tiers: [1, 2, 3, 4], pay: [150, 150, 150, 400], ids: () => MAP_DATA.map(m => String(m.n)), name: id => mapData(+id).name, sub: () => t("mastery.mapSub"),
            have: id => { const B = mapData(+id).bosses.end, mk = profile.mapMakes[id] || 0; return (profile.bossLog[B] ? 1 : 0) + (profile.flawless[B] ? 1 : 0) + (mk >= 100 ? 1 : 0) + (mk >= 300 ? 1 : 0); } },
    boss: { tiers: [1, 5, 20, 50], pay: [100, 250, 500, 1000], ids: () => BOSS_IDS.slice(), have: id => profile.bossLog[id] || 0, name: id => BOSS_INFO[id].name, sub: id => BOSS_INFO[id].tell },
    minis: { tiers: [1, 10, 25, 50], pay: [100, 250, 500, 1000], ids: () => MINI_IDS.slice(), have: id => (profile.modes[id] || {}).runs || 0, name: id => t(`mode.${id}.name`), sub: id => t(`mode.${id}.rule`) },
    power: { tiers: [1, 10, 30, 75], pay: [100, 250, 500, 1000], ids: () => POWER_IDS.slice(), have: id => (profile.powerLog || {})[id] || 0, name: id => POWERS[id].name, sub: id => POWERS[id].tip }
  };
  ...
  function renderMastery() {
    ...
    for (const id of M.ids()) {
      const row = h("div", { class: "m-row", data: { m: `${mUI.cat}:${id}` } }, h("div", { class: "m-head" }, h("b", {}, M.name(id)), h("span", { class: "m-have" }, mUI.cat === "map" ? "★".repeat(M.have(id)) + "☆".repeat(4 - M.have(id)) : `${fmtN(M.have(id))}×`)), h("p", {}, M.sub(id)));
